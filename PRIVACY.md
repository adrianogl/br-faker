...
Three settings, and only because you set them:
...
These live in `chrome.storage.sync`, which means Chrome may sync them between
...
or sent anywhere. The extension holds no host permissions, so it has no access to
any site until you ask it to act on the tab in front of you.
