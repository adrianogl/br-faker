...
npm run icon      # regenerate workflow/icon.png
npm run workflow  # build the icon, bundle, and package the .alfredworkflow
```

The icon is generated by `scripts/build-icon.mjs` rather than committed as an opaque binary someone has to take on trust — shapes are signed distance fields and the PNG is encoded on top of Node's built-in zlib, so changing the artwork is a code review, not a file swap.
...
| `scripts/build-icon.mjs` | Draws `workflow/icon.png` — no image dependencies |
